...
Nur-lesender Zugriff, Werte nie in Git. Postfach lt. team-mail/D001: **dimitri.john83@gmail.com**, Zugangsdaten = vorhandene Mail-Einrichtung — das Skript fällt automatisch auf `SMTP_USER`/`SMTP_PASS` zurück und meldet beim Start, welches Postfach es nutzt. Test in neuem Fenster: `python team-mail\tools\mail_digest.py --tage 1`. Abweichende Werte: `setx MAIL_IMAP_HOST/USER/PASS` (App-Passwort: Google-Konto → Sicherheit → App-Passwörter). Das Skript erzwingt `readonly=True` (nie löschen/verschieben/markieren/senden — Guardrail 1). Rohdaten und Digests bleiben im lokalen Repo `team-mail` (kein Remote, `.kein-remote`). Widerruf: App-Passwort im Google-Konto löschen bzw. `setx MAIL_IMAP_PASS ""`.
