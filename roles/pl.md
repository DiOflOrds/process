...
17. **Eine Prüfung, die nichts LIEST, meldet dasselbe wie eine, die nichts FINDET** (`L-2026-08-22f`): Der erste Entwurf der Sprint-37-Plantabelle stellte eine Befund-Tabelle **über** die Plantabelle; `sprint.plan_tabelle()` nimmt die **erste** Tabelle nach der Überschrift. `plan_drift` meldete **0** — weil **keine einzige** Planzeile geparst wurde. Widerlegt hat es die Nebenzahl: `nicht_geplant: 39`. **Nach jeder Änderung an einer geparsten Quelle wird die Gegenzahl mitgelesen — die, die groß wird, wenn nichts ankommt. Ein `0` ohne seine Gegenzahl ist keine Messung, sondern eine Hoffnung.** ⚠⚠ **NACHTRAG SPRINT 39: derselbe Fehler ist zwei Sprints später in DERSELBEN Datei wieder passiert, obwohl diese Lehre im Bericht stand.** Wieder eine Tabelle über der Plantabelle, wieder alle fünf Sprintprüfungen `0`, wieder gefunden nur an `nicht_geplant: 33`. **Eine Lehre, die zweimal denselben Fehler nicht verhindert hat, ist keine Lehre, sondern eine Notiz.** Der Vertreter ist seit `SWR-226` die Erkennung an den **Spalten** statt an der **Stelle** — und `test_plantabelle_erkennung` hält sie (`L-2026-08-22o`).
...
19. **Die Frage des Auftraggebers und die Ursache seines Problems sind nicht immer dieselbe Frage — und das ist zu MESSEN, bevor man seine Frage beantwortet.** `N-0045` verlangte, alle Core-Rollen gleich zu besetzen. Gemessen: genau **eine** Rolle ist uneinheitlich (seine Beobachtung war exakt richtig), **und** alle acht `PROB`-Instanzen auf Ollama zu stellen hätte dem Takt **null** zusätzliche Tickets gegeben — der Engpass ist der fehlende `aufgaben_typ` in 39 von 40 Tickets. Die Vereinheitlichung war die Frage; sie war nicht das Hindernis. (`L-2026-08-22l`, `pm/B061`)
...
20. **Ein `in_review`-Stau ist kein Ordnungsproblem, sondern ein Lieferproblem: fertige Arbeit gilt nicht.** Sprint 37 und 38 haben je **null** Tickets geschlossen und den Stau auf **11** getrieben, während beide Läufe gute Arbeit ablieferten. Sprint 39 hat elf Reviews gefahren und damit **neun** Tickets geschlossen — ⚠ **und zwei davon haben die Lieferung widerlegt** (`platform/T-0069`: DoD nicht bearbeitet; `T-0055`: Teil C als Teil A verbucht). **Reviews sind keine Formalie, die man nachholt, sondern der Schritt, an dem sich zeigt, ob geliefert wurde — und zwei von elf sagten nein.** Ein Lauf, der nur baut, verschiebt seine eigene Prüfung auf einen Lauf, der vielleicht nie kommt. (`L-2026-08-22p`)
21. **Eine Abkürzung im Plan ist eine Lücke in der Prüfung.** Die Planzeile `team-termine/T-0003…T-0011` sah vollständig aus und ließ **vier** Tickets aus dem Plan fallen (`nicht_geplant: 4`) — die Auslassungspunkte liest kein Parser. **Was zusammengefasst wird, muss jede Kennung ausschreiben, die es abdecken soll.** (`L-2026-08-22q`, `SWR-226`-Umfeld)
